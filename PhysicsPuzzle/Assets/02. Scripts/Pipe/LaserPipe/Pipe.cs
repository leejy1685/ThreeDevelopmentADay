...
using _02._Scripts.Pipe.Common.ParticleSystem;
using UnityEngine;
...
namespace _02._Scripts.Pipe.LaserPipe
{
    ...
    public class Pipe : MonoBehaviour, IInteractable
    {
        ...
        private List<Port> _ports;  // 이 파이프에 포함된 모든 Port들
        private Dictionary<Port, LineRenderer> _portLineMap; // 포트별로 캐싱된 LineRenderer
        private Dictionary<Port, int> _lastShotFrameByPort; // 포트별로 마지막으로 레이저를 발사한 프레임 기록
        
        private void Awake()
        {
            _ports = GetComponentsInChildren<Port>().ToList();
            _portLineMap = new Dictionary<Port, LineRenderer>();
            _lastShotFrameByPort = new Dictionary<Port, int>();

            // 포트마다 LineRenderer 한 개씩 생성·캐싱
            foreach (var p in _ports)
            {
                p.parentPipe = this;
                _lastShotFrameByPort[p] = -1;
        ...
        private void LateUpdate()
        {
            ...
            foreach (var kv in _portLineMap)
            {
                var port = kv.Key;
                var lr = kv.Value;
                if (Time.frameCount > _lastShotFrameByPort[port])
                {
                    lr.enabled = false;
        ...
        {
            // 같은 프레임에 중복 처리 방지
            if (_lastShotFrameByPort[hitPort] == Time.frameCount) return;
            _lastShotFrameByPort[hitPort] = Time.frameCount;

            // 입력된 포트 제외하고 나머지 포트들로 레이저 분기
            for (int i = 0; i < _ports.Count; i++)
            {
                var outPort = _ports[i];
        ...
        private void ShootFromPort(Port port, LaserBeam incomingBeam)
        {
            ...
            _lastShotFrameByPort[port] = Time.frameCount;
        }
